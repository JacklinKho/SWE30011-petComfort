...
def process_state_message(payload):
    ...
    # Insert data into Dog_Control_Table for cloud database
    try:
        with cloudDB.cursor() as cursor:
            ...
            if existing_record:
                cursor.execute(
                    "UPDATE Dog_Control_Table SET lightState = %s, fanState = %s, windowState = %s WHERE dogControlID = 1",
...
def process_threshold_message(payload):
    ...
    # Insert data into Dog_Adjust_Table for cloud database
    try:
        with cloudDB.cursor() as cursor:
